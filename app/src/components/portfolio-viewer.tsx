...
import type { PortfolioData, Intro, Slide } from "@/lib/types";
...
export function PortfolioViewer({ data }: PortfolioViewerProps) {
  ...
  const handleRefresh = useCallback((newIntro: Intro, newSlides: Slide[]) => {
    setIntro(newIntro);
    setSlides(newSlides);
  }, []);
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="pt-16">
        <div className="mx-auto max-w-4xl xl:max-w-7xl px-6 py-12 pb-20">
          {currentIndex === 0 && <IntroSlide intro={intro} slides={slides} onImageError={requestRefresh} />}
          {currentIndex > 0 && (
